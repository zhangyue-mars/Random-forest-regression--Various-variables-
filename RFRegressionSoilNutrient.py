...
if __name__ == "__main__":
    ...
    # 影响因子-样本数据
    listSample = [[f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12] for
                  f1, f2, f3, f4, f5, f6, f7, f8, f9, f10, f11, f12
                  in zip(dfSample['Elevation'],
                         dfSample['Slope'], dfSample['Aspect'], dfSample['Planc'],
                         dfSample['Profic'], dfSample['TPI'], dfSample['SR'],
                         dfSample['TWI'], dfSample['NDVI'], dfSample['MAP'],
                         dfSample['MAT'], dfSample['EVI'])]
    ...
    outputDataPath = r'E:\\RemoteSensing\\XYKH\\随机森林回归土壤水分数据\\2018gz\\resultTIFF\\RF_' + calIndex + r'\\'# 每次计算需要修改这里的文件夹名（需要提前新建文件夹），以免覆盖文件
    ...
    # 创建随机森林回归
    # 随机森林参数参考网页【https://blog.csdn.net/Ling_Ze/article/details/126050941】
    # model_RandomForestRegressor = svm.SVR()
    # model_RandomForestRegressor = RandomForestRegressor(n_estimators=100, random_state=0)  # 这里使用100个决策树
    model_RandomForestRegressor = RandomForestRegressor(n_estimators=2000, max_features = 8,  n_jobs=4, min_samples_leaf=10)
    # 调用fit训练
    model_RandomForestRegressor.fit(np.array(X_train).reshape(len(X_train),-1), np.array(y_train).reshape(len(X_train), -1))
    # 预测未知数据
    y_pred = model_RandomForestRegressor.predict(X_test)
    # print(results[0])
    # ****************************计算精度**************************** #
    importances = list(model_RandomForestRegressor.feature_importances_)
    print("\n特征权重值：", importances)
    ...
    for i in tqdm(range(0, nYsize)):
        ...
        for j in range(0, nXsize):
            ...
            # else:
                ...
                features = [ElevationValue, SlopeValue, AspectValue, PlancValue, ProficValue, TPIValue, SRValue,
                            TWIValue, NDVIValue, MAPValue, MATValue, EVIValue]  # 每个像素的经纬度
                row.append(features)
                # print(features)
    ...
    predictiveFeatures = np.reshape(listSampleTemp,(-1,12))#  14
    ...
    # **********************************预测未知数据********************************** #
    resultsLabel = model_RandomForestRegressor.predict(predictiveFeaturesScale)
    # print(type(resultsLabel), resultsLabel)
    arrayRFresult = resultsLabel.reshape(rows, -1)

    # 保存结果为tiff
    outputName = r'RF_' + calIndex + r'_regression'
    ...
    # 保存结果为tiff
    outputNameRRF = r'ResidualRF_' + calIndex + r'_regression'
